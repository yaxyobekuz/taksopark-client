...
import { useDriverDocumentUpdate } from "../../hooks/useDriverDocumentMutations";

const toDateInput = (date) =>
  date ? new Date(date).toISOString().slice(0, 10) : "";

const DriverDocumentEditModal = ({ close, driverId, document }) => {
  ...
  const { mutate, isPending } = useDriverDocumentUpdate();

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!driverId || !document?._id) return;
    mutate(
      { driverId, docId: document._id, ...state },
      { onSuccess: () => close() },
    );
  };
...
export default DriverDocumentEditModal;
