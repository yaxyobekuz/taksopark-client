...
const DriverDetailPage = () => {
  ...
  if (!driver)
    return <p className="text-sm text-muted-foreground">Haydovchi topilmadi</p>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-3 text-sm">
        <Link
          to={`/owner/payments?driverId=${driver._id}`}
          className="text-primary hover:underline"
        >
          To'lovlar
        </Link>
        <Link
          to={`/owner/fines?driverId=${driver._id}`}
          className="text-primary hover:underline"
        >
          Jarimalar
        </Link>
        <Link
          to={`/owner/damages?driverId=${driver._id}`}
          className="text-primary hover:underline"
        >
          Zararlar
        </Link>
        <Link
          to={`/owner/cycles?driverId=${driver._id}`}
          className="text-primary hover:underline"
        >
          Tsikllar
        </Link>
        <Link
          to={`/owner/payments/reports/statement/${driver._id}`}
          className="text-primary hover:underline"
        >
          Hisobot
        </Link>
