...
import { useDamagePaymentCreate, useDamagePaymentsByDamage, useDamagePaymentDelete } from "../../hooks/useDamagePayments";

const DamagePayModal = ({ close, damage }) => {
  const driverId = damage?.driver?._id || damage?.driver;
  const { data: balance } = useDriverBalanceQuery(driverId);
  const { data: payments = [] } = useDamagePaymentsByDamage(damage?._id);
  const { mutate, isPending } = useDamagePaymentCreate();
  const deleteMutation = useDamagePaymentDelete();

  const remaining = useMemo(() => {
    if (!damage) return 0;
    return Math.max(0, damage.amount - damage.paidAmount);
  }, [damage]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!damage?._id || !amount || insufficient || tooMuch) return;
    mutate(
      {
        damageId: damage._id,
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-md border bg-muted/30 p-3 text-sm space-y-1">
        <div className="flex justify-between"><span className="text-muted-foreground">Jami zarar:</span> <span className="font-medium">{formatMoney(damage.amount)}</span></div>
        <div className="flex justify-between"><span className="text-muted-foreground">To'langan:</span> <span>{formatMoney(damage.paidAmount)}</span></div>
